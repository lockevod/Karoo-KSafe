...
| 28 (hydration) | — | `1136e7` (v2.2.0, 13 of them inside one hour) |
...
- **Lifetime.** The trackers are instantiated once by `KSafeExtension` and `start()`ed per ride, and the counter is in-memory. A **new ride** reseeds `lastRealLogMs`, so the anchor mismatches and the ladder resets — correct. An **in-process pause/resume** restores the same `lastRealLogMs`, so the anchor matches and the back-off is *preserved* — also correct (the rider is mid-ride and still not logging). Only a process restart drops it. Persisting it would need a `CarbFuelingState`/`HydFuelingState` field plus a `CONFIG_VERSION` bump; not worth it for that one case.
